...
server <- function(input, output, session) {
  ...
  startYear <- reactive({
    week52_yearweek(input$dateRange[1])$year
  })
  
  endWeek <- reactive({
    week52_yearweek(input$dateRange[2])$week
  })
  ...
  output$interactive_output <- renderUI({
    if (input$graph == "hpai_map") {
      ...
      plotlyOutput("forecasting_plotly", height = 760)
  ...
  # This keeps the interactive Plotly version aligned with the original
  ...
  model_fit_data_plotly <- reactive({
    ...
    comp_wide <- tibble(
      ...
      within = as.numeric(fit_df[[idx_own]]),
      between = as.numeric(fit_df[[idx_ne]])
    ) %>%
      mutate(
        cum_endemic = endemic,
  ...
  model_fit_static_ggplot <- reactive({
    ...
    ggplot() +
      ...
      labs(title = d$title, x = "Time", y = "No. detected cases", fill = NULL) +
  ...
  forecast_fan_data <- reactive({
    ...
    last_observed_week_start <- lubridate::floor_date(
      as.Date(minDate) + lubridate::weeks(prev_idx - 1),
      unit = "week",
      week_start = 1
    )
    ...
    df <- tibble(
      ...
      mutate(
        mean_hover = paste0(
          "Forecast week starting: ", format(date, "%d %b %Y"),
          "<br>Mean forecast: ", round(mean, 1),
          "<br>90% interval: ", round(q05, 1), " - ", round(q95, 1),
          "<br>80% interval: ", round(q10, 1), " - ", round(q90, 1),
    ...
    obs_df <- tibble(
      ...
      obs_hover = paste0(
        "Last observed week starting: ", format(observed_date, "%d %b %Y"),
        "<br>Observed detections: ", observed,
    ...
    list(
      ...
      df = df,
      obs = obs_df,
      band_90 = make_band(df, "q05", "q95"),
      band_80 = make_band(df, "q10", "q90"),
      band_50 = make_band(df, "q25", "q75"),
      ymax = ymax,
      xmin = obs_df$date - lubridate::days(4),
      xmax = max(df$date) + lubridate::days(4),
  ...
  forecasting_plotly <- reactive({
    ...
    plot_ly() %>%
      add_trace(
        ...
        fill = "toself",
        name = "90% interval",
        line = list(color = "rgba(254, 232, 200, 0.80)", width = 1),
        fillcolor = "rgba(254, 232, 200, 0.75)",
        hoverinfo = "skip",
        showlegend = TRUE
      ) %>%
      add_trace(
        ...
        fill = "toself",
        name = "80% interval",
        line = list(color = "rgba(253, 187, 132, 0.85)", width = 1),
        fillcolor = "rgba(253, 187, 132, 0.70)",
        hoverinfo = "skip",
        showlegend = TRUE
      ) %>%
      add_trace(
        ...
        fill = "toself",
        name = "50% interval",
        line = list(color = "rgba(227, 74, 51, 0.90)", width = 1),
        fillcolor = "rgba(227, 74, 51, 0.55)",
        hoverinfo = "skip",
        showlegend = TRUE
      ) %>%
      add_lines(
      ...
      # Separate legend-only trace so the mean line remains white in the fan,
      # but the legend key is visible against the white background.
      add_trace(
        x = c(NA),
        y = c(NA),
        type = "scatter",
        mode = "lines",
        name = "Mean forecast",
        line = list(color = "grey25", width = 3),
        hoverinfo = "skip",
        showlegend = TRUE
      ) %>%
      add_markers(
      ...
      layout(
        ...
        yaxis = list(
          title = "No. detected/predicted cases",
          range = c(0, d$ymax * 1.15),
          fixedrange = FALSE
        ),
        legend = list(
          orientation = "h",
  ...
  forecast_fan_static_ggplot <- reactive({
    ...
    ggplot() +
      geom_ribbon(
        data = d$df,
        aes(x = date, ymin = q05, ymax = q95, fill = "90% interval"),
        alpha = 0.75
      ) +
      geom_ribbon(
        data = d$df,
        aes(x = date, ymin = q10, ymax = q90, fill = "80% interval"),
        alpha = 0.70
      ) +
      geom_ribbon(
        data = d$df,
        aes(x = date, ymin = q25, ymax = q75, fill = "50% interval"),
        alpha = 0.55
      ) +
      geom_line(
        data = d$df,
        aes(x = date, y = mean, colour = "Mean forecast"),
        linewidth = 1.2
      ) +
      geom_point(
        data = d$obs,
        aes(x = date, y = observed, shape = "Last observed"),
        colour = "black",
        fill = "black",
        size = 2.5
      ) +
      scale_fill_manual(
        ...
        breaks = c("90% interval", "80% interval", "50% interval"),
        name = NULL
      ) +
      scale_colour_manual(
        values = c("Mean forecast" = "grey25"),
        breaks = c("Mean forecast"),
        name = NULL
      ) +
      scale_shape_manual(values = c("Last observed" = 16), name = NULL) +
      coord_cartesian(
        xlim = c(d$xmin, d$xmax),
        ylim = c(0, d$ymax * 1.15)
      ) +
      scale_x_date(
        name = "Forecast week",
        breaks = d$breaks,
...
fixed scale")
  ...
  AllCountry_forecasting <- reactive({
    ...
    title(xlab = "", line = 3.0, cex.lab = 1.55)
  ...
  output$info <- renderText({
    if (input$graph == "hpai_map") {
      ...
      if (identical(input$forecast_display, "map")) {
        ...
        paste0(
          "Simulation-based 4 week forecast based on the modelling framework described in Kjær et al. (2023). The plot shows the weekly number of predicted highly pathogenic avian influenza (H5 subtype) detections aggregated over all 37 countries included in the model. The shaded fan shows simulation uncertainty: the darkest band is the central 50% interval, the medium band is the central 80% interval, and the lightest band is the central 90% interval. The white line shows the mean forecast. The black dot shows the last observed value used to initialise the forecast."
        )
      } else {
        unit <- which(districts2plot2 == input$forecasting_options)
        paste0(
          "Simulation-based 4 week forecast for ", districts2plot2[unit],
          ". The shaded fan shows simulation uncertainty: the darkest band is the central 50% interval, the medium band is the central 80% interval, and the lightest band is the central 90% interval. The white line shows the mean forecast. The black dot shows the last observed value used to initialise the forecast."
        )
      }
    }
  })
  ...
  output$downloadPlot <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (input$graph == "forecasting" && identical(input$forecast_display, "map")) {
        ...
        print(forecast_fan_static_ggplot())
      } else if (input$graph == "country_modelfit") {

...
yw_year <- function(yw) {
  lubridate::isoyear(tsibble::as_date(yw))
}

yw_week <- function(yw) {
  lubridate::isoweek(tsibble::as_date(yw))
}
...
shinyApp(
  ...
  server <- function(input, output) {
    ...
    allCountry_modelfit<- reactive({
      myPlot <-plot(final_model, type = "fitted", total = TRUE,hide0s = TRUE, ylab="No. detected cases", xlab="Years and quarters",ylim=c(0,(max(rowSums(observed(final_model$stsObj[start():end(), ]), na.rm=T)))+50),par.settings = list(mar = c(7, 5, 4.1, 2.1)), xaxis=list(epochsAsDate=TRUE),start = c(yw_year(startYW()), yw_week(startYW())), end   = c(yw_year(endYW()),   yw_week(endYW())),col=c('brown2','#4292C6','orange'),names="Summed all countries", legend.args=list(x="topright", inset=c(-0.1,0.003), horiz=T,legend = c("Epidemic component,between-country","Epidemic component,within-country", "Endemic"),col=c('orange','#4292C6','brown2')))
      print(myPlot)
      recordPlot()
    })

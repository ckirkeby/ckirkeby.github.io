...
#
# This file is part of the Shiny app for the ENIGMA HPAI model version 1.x.
...
## ---- YEARWEEK HELPERS (YEAR-SAFE) ----
to_yw <- function(x) tsibble::yearweek(x)
yw_year <- function(yw) {
  lubridate::isoyear(as.Date(yw))
}
...
required_objs <- c(
  ...
  "europe_data_weekly", "europeanCountries", "europe_data",
  "districts2plot2", "districts2plot",
  "AI_sts", "final_model",
  "atChange", "atMedian"
)
...
if (length(missing_objs) > 0) {
  stop(
    "Missing required objects after qload(): ",
    paste(missing_objs, collapse = ", "),
    call. = FALSE
  )
}

## ---- FOOTNOTE FOR DOWNLOADS ----
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ## NOTE:
      ## If you want to GUARANTEE a full 4-week forecast (plus 1-week reporting delay),
      ## restrict max selectable end date to endDate - 35 days:
      ## max = as.Date(endDate) - 35
      dateRangeInput(
        "dateRange",
        paste0(
          "Select Date Range (needs to be within ",
          strftime(minDate, format = "%d/%m/%Y"), " and ",
      ...
      conditionalPanel(
        ...
        selectInput(
          "predictions_options",
          "Select country (only countries with > 10 detections total are shown)",
      ...
      div(img(src = "EN-Co-Funded-by-the-EU_POS-1024x215.png", height = 40, width = 160),
          img(src = "EUP AH&W_color.png", height = 60, width = 160)),
      HTML('<hr style = "border-color: #800000; height: 5px;width: 100%">'),
      ...
      br(),
      div(
        img(src = "gplv3-with-text-136x68.png", height = 40, width = 80),
        br(),
...
server <- function(input, output, session) {
  ...
  ## YEARWEEK REACTIVES (YEAR-SAFE)
  startYW <- reactive({ to_yw(input$dateRange[1]) })
  endYW   <- reactive({ to_yw(input$dateRange[2]) })
  
  ## Numeric indices used by sts objects: keep your original approach but ensure bounds
  start <- reactive({
    as.numeric(floor(difftime(input$dateRange[1], as.Date(minDate), units = "weeks") + 1))
  })
  ...
  observeEvent(input$dateRange, {
    ...
    if (start() < 1 || end() < 1 || start() > nT || end() > nT || start() > end()) {
      showNotification(
        paste0(
          "Selected date range is outside the model time window. ",
          "Please choose dates within the available range."
        ),
        type = "error",
        duration = 6
      )
  ...
  europe_mapData1 <- reactive({
    europe_data_weekly %>%
      filter(yearWeek >= startYW() & yearWeek <= endYW()) %>%
      group_by(ADM0_A3) %>%
  ...
  hpai_table <- reactive({
    myTable <- data.frame(
      keyName = names(colSums(observed(final_model$stsObj)[start():end(), ])),
      value = colSums(observed(final_model$stsObj)[start():end(), ])
    ) %>%
  ...
  hpai_timeseries <- reactive({
    ...
    myPlot <- plot(
      ...
      ylim = c(0, max(rowSums(observed(AI_sts[start():end(), ]), na.rm = TRUE)) + 100)
    )
    print(myPlot)
    recordPlot()
  })
  ...
  AllCountry_forecasting <- reactive({
    ...
    myPlot <- plot(
      ...
      par.settings = list(pch = 1, cex = 1.2, mar = c(7, 5, 4.1, 2.1))
    )
    title(ylab = "No. of detected/predicted cases", line = 3, cex.lab = 1.5, cex.axis = 1.5)
    grid()
    print(myPlot)
    recordPlot()
  })
  ...
  country_forecasting <- reactive({
    ...
    myPlot <- plot(
      ...
      type = "fan",
      ylim = c(0, quantile(sim()[, districts2plot[unit], ], 0.99)),
      means.args = list(),
      xaxis = list(
        epochsAsDate = TRUE,
        xaxis.tickFreq = list("%d" = atChange, "%m" = atChange),
  ...
  ## Plot name for download
  getPlotName <- reactive({
    if (input$graph == "country_modelfit") {
      if (input$predictions_options == "Summed all countries") "allCountry_modelfit()"
      else "country_modelfit()"
    } else if (input$graph == "forecasting") {
      if (input$forecasting_options == "Summed all countries") "AllCountry_forecasting()"
      else "country_forecasting()"
    } else {
      paste0(input$graph, "()")
    }
  })
  ...
  output$graph <- renderPlot({
    if (input$graph == "hpai_map") replayPlot(req(hpai_map()))
  ...
  output$info <- renderText({
    ...
    if (input$graph == "hpai_map") {
      return(paste0(
        ...
        " and ",
        strftime(input$dateRange[2], format = "%d/%m/%Y"),
        ", in 37 European countries shown geographically as total number of detections (left), number of detections per 10,000 km² (middle), and as individual detection locations (right). Below is a table of the total number of detections per country ordered from highest to lowest."
      ))
    }
    ...
    if (input$graph == "hpai_timeseries") {
      return(paste0(
        ...
        " and ",
        strftime(input$dateRange[2], format = "%d/%m/%Y"),
        ", summed over 37 European countries."
      ))
    }
    ...
    if (input$graph == "country_modelfit") {
      if (input$predictions_options == "Summed all countries") {
        return(paste0(
          ...
          " to ",
          strftime(input$dateRange[2], format = "%d/%m/%Y"),
          ". The plot shows the relative contribution of model components based on the modelling framework described in Kjær et al. (2023). Dots show the actual counts of reported highly pathogenic avian influenza (H5 subtype) detections in domestic and wild birds. Note that zero/missing detections have been omitted."
        ))
      } else {
        unit <- which(districts2plot2 == input$predictions_options)
        return(paste0(
          "Model fit for ",
          districts2plot2[unit],
          " shown for ",
          strftime(input$dateRange[1], format = "%d/%m/%Y"),
          " to ",
          strftime(input$dateRange[2], format = "%d/%m/%Y"),
    ...
    if (input$graph == "forecasting") {
      last_obs_yw <- endYW() - 1
      last_obs_week <- yw_week(last_obs_yw)
      last_obs_year <- yw_year(last_obs_yw)
      
      if (input$forecasting_options == "Summed all countries") {
        return(paste0(
          "Simulation-based 4 week forecast based on the modelling framework described in Kjær et al. (2023). ",
          "The plots show weekly number of predicted highly pathogenic avian influenza (H5 subtype) detections aggregated over all 37 countries included in the model. ",
          "The fan chart represents the 10%, 50% and 90% quantiles of the simulations (N=500) each week; their mean is displayed as a white line. ",
          "The black dot to the left of the graph depicts the number of detections from ISO week ",
          last_obs_week, " in ", last_obs_year, ". ",
          "Thus, we used the second to last week of data (to account for delays in WOAH-WAHIS reporting) to forecast ahead. For further details, see Kjær et al. (2023)."
        ))
      } else {
        unit <- which(districts2plot2 == input$forecasting_options)
        return(paste0(
          "Simulation-based 4 week forecast based on the modelling framework described in Kjær et al. (2023). ",
          "The plots show weekly number of predicted highly pathogenic avian influenza (H5 subtype) detections for ",
          districts2plot2[unit], ". ",
          "The fan chart represents the 10%, 50% and 90% quantiles of the simulations (N=500) each week; their mean is displayed as a white line. ",
          "The black dot to the left of the graph depicts the number of detections from ISO week ",
          last_obs_week, " in ", last_obs_year, ". ",
